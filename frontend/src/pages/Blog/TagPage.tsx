...
import { Calendar, ChevronLeft, ChevronRight, Eye, Hash, Tag } from 'lucide-react';
...
const TagPage = () => {
  ...
  const { tagCloud, fetchTagCloud, isFetchingTags } = useTag();
  ...
  const fetchTagPosts = useCallback(
    async (tagId: number, page: number) => {
      setIsLoading(true);
      try {
        const res = await getTagPostsApi(tagId, {
          pageable: {
            currentPage: page,
            pageSize,
  ...
  useEffect(() => {
    if (currentTag) {
      fetchTagPosts(currentTag.id, currentPage);
    }
  }, [currentTag, currentPage, fetchTagPosts]);
            ...
            <Card>
              ...
              <CardContent className='space-y-2 text-sm'>
                <div className='flex justify-between'>
                  ...
                  <span className='font-medium'>{tagCloud.length}</span>
                </div>
                <Separator />
                <div className='flex justify-between'>
                  <span className='text-muted-foreground'>文章总数</span>
                  <span className='font-medium'>
                    {tagCloud.reduce((sum, tag) => sum + tag.postCount, 0)}
                  </span>
                </div>
              </CardContent>
            </Card>
